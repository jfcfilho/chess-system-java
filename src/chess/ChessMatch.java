...
public class ChessMatch {
	...
	private Piece makeMove(Position source, Position target) {
		...
		// fazendo movimento do Roque pequeno (a partir do Rei para a Torre)
		...
		// tratamento para jogada enPassant (para peça Peão)
	...
	private void undoMove(Position source, Position target, Piece capturedPiece) {
		...
		// desfazendo movimento do Roque pequeno (a partir do Rei para a Torre)
		if (p instanceof King && target.getColumn() == source.getColumn() + 2) {
			Position sourceT = new Position(source.getRow(), source.getColumn() + 3);
			Position targetT = new Position(source.getRow(), source.getColumn() + 1);
	...
	private boolean testCheckMate(Color color) {
		...
		List<Piece> listPieces = piecesOnTheBoard.stream().filter(x -> ((ChessPiece) x).getColor() == color)
				.collect(Collectors.toList());
